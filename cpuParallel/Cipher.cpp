...
Cipher operator*(Cipher a, Cipher b) {
    ...
    int nb_bits = a.numberOfBits * 2;// + b.numberOfBits;
    Cipher out(nb_bits);
...
#pragma omp declare reduction \
    ...
    initializer(omp_priv=Cipher(32))
    omp_set_num_threads(Cipher::nThreads);
#pragma omp parallel for schedule(static) reduction(OMP_CIPHER_SUM:out)
#endif
    for (int i = 0; i < b.numberOfBits; ++i) {
//        cout << "omp_get_num_threads(): " << omp_get_num_threads() << " id: " << omp_get_thread_num() << endl;
//        cout <<
...
Cipher cipherAND(Cipher a, Cipher b) {
    Cipher result(a.numberOfBits);
#pragma omp parallel for
    for (int i = 0; i < a.numberOfBits; ++i) {
        bootsAND(&result.data[i], &a.data[i], &b.data[i], Cipher::bk);
    }
    return result;
}
