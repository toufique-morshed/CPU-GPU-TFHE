...
EXPORT void lweNoiselessTrivial_16(LweSample_16* result, Torus32 mu, int bitSize){
...
EXPORT void lweSubTo_16(LweSample_16* result, const LweSample* sample, int startIndex, int endIndex, int bitIndex,
                        const LweParams* params) {
//    cout << "lweSubTo_16" << endl;
    const int n = params->n;
    const Torus32* __restrict sa = sample->a;
    Torus32* __restrict ra = result->a;
//    cout << "params->n: " << params->n << endl;
//    cout << "params->n: " << pa << endl;
//
    for (int i = 0; i < n; ++i) {
        ra[bitIndex * n + i] -= sa[i];
    }
    //test morshed start
//    cout << "new: ";
//    for (int i = 0; i < 10; ++i) {
...
//    cout << endl;
    ...
    result->b[bitIndex] -= sample->b;
    result->current_variance[bitIndex] += sample->current_variance;
}
...
///new
EXPORT void lweAddTo_16(LweSample_16* result, const LweSample_16* a, const LweSample_16* b, int bitSize, const LweParams* params){
    const int n = params->n;

    for (int i = 0; i < n*bitSize; ++i) {
        result->a[i] = result->a[i] + a->a[i] + b->a[i];
        if (i < bitSize) {
            result->b[i] = result->b[i] + a->b[i] + b->b[i];
            result->current_variance[i] = result->current_variance[i] + a->current_variance[i] + b->current_variance[i];
        }
    }
}
